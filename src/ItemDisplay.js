...
class ItemDisplay extends Component {
  ...
  indexHandler = (offset) => {
    if (this.state.carouselIndex === 0 && offset === -1) {
      return;
    } else if (this.state.carouselIndex === this.props.item.images.length - 1 && offset === 1) {
      return;
    } else {
      this.setState({
    ...
    return (
      <div className="display-wrap">
        <div className="display-content">
          ...
          {
            this.props.item.details.map(
              (detail) => (<p className="display-detail">{detail}</p>)
            )
          }
          <div className="display-image-wrap">
            <div className="arrow-wrap">
              <MdChevronLeft
                className={this.state.carouselIndex === 0
                  ? "icon-disabled"
                  : "icon"
}
            ...
            <img
              ...
              onClick={() => this.props.setOverlayImage("photos/" + this.props.item.images[this.state.carouselIndex])
}/>{" "}
            <div className="arrow-wrap">
              <MdChevronRight
                className={this.state.carouselIndex === this.props.item.images.length - 1
                  ? "icon-disabled"
                  : "icon"
}
                onClick={() => this.indexHandler(1)}/>
